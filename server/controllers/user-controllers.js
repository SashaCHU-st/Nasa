...
const authJWT = (req, res, next) => {
  ...
  jwt.verify(token, process.env.JWT_SECRET_KEY, (err, decoded) => {// verificate paramenters token and +secret key
...
const getUsers = async (req, res, next) => {
  ...
  const usersWithFavoritesCount = users.map(user => ({// mapping users one by one by user
    id: user.id,
    name: user.name,
    email: user.email,
...
const createUser = async (req, res, next) => {
  ...
  if (!error.isEmpty()) {// if false then
    // console.log(error);
...
const login = async (req, res, next) => {
  ...
  if (!hasUser || hasUser.password !== password) {// if pass is not belongs to user
