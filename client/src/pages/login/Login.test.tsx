...
describe("Login Component", () => {
  ...
  it("submits login form successfully", async () => {
    ...
    vi.spyOn(global, "fetch").mockImplementationOnce(() =>
      /// mocking fun globaly, ensure it implements once
      Promise.resolve({
        // miock implemet returs a Promise that rsponse with succsesull
        ok: true, // all good status 200
        json: () =>
          /// retrun  a promise that return with mock data userId and token
          Promise.resolve({
            userId: "123",
            token: "mockToken",
          }),
    ...
    await waitFor(() =>
      expect(mockLogin).toHaveBeenCalledWith("123", "mockToken")
    );
  ...
  it("Wrong credenital;Logging in failed, wrong credentials.", async () => {
    ...
    vi.spyOn(global, "fetch").mockImplementationOnce(() =>
      Promise.resolve({
        ok: false,
        json: async () => ({
          message: "Logging in failed, wrong credentials.",
        }),
      } as Response)
    );
    ...
    fireEvent.click(screen.getByText(/LOGIN/i));
    await waitFor(() =>
      screen.getByText(/Logging in failed, wrong credentials/i)
    );
  });
});
