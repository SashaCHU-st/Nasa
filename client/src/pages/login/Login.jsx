...
const Auth = () => {
  ...
  const authSubmitHandler = async (event) => {
    ...
    if (isLoginMode) {
      ...
      try {
        setError(null);
        const response = await fetch("https://nasa-57eq.onrender.com//api/users/login", {
          method: "POST",
          headers: {
        ...
        setIsLoading(false);
        auth.login(); // Логиним пользователя
        navigate(from); // Перенаправляем на исходный маршрут
      } catch (err) {
      ...
      // Handle signup
      try {
        const response = await fetch("https://nasa-57eq.onrender.com//api/users/signup", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
